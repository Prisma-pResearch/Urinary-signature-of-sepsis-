...
textMining <- function(geneset){
  ...
  for (i in 1:length(keyterms))
    for(j in 1:length(geneset)){
      ...
      write.csv(rSearch$ids, paste("S:/PRISMAP Studies/PROJECTS/C2 Urinary Signature of Sepsis/3 Results/Transcriptome Analysis Console/WithoutUTI_onlyRMA/Results/Text Mining_selected probes_without Lasso_only Sepsis/", geneset[j], keyterms[i], ".csv", sep="")) #write the pubmed ids in a csv file
    }
  return(rSearch)
}