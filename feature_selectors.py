...
def Boruta_fs(X_train, y_train):
    ...
    selector = BorutaPy(estimator, n_estimators='auto', verbose=2, random_state=0)#perc=100, max_iter=100, two_step=True
    
    selector.fit(X_train.values, y_train.values)
    feature_names = X_train.columns.values
    df_rank = pd.DataFrame({'Rank': selector.ranking_, 'Features': feature_names}) #finding ranked list
    
    #Saving the confirmed features
    confirmed_indices = np.where(selector.ranking_ == 1)
    confirmed_names = X_train.columns.values[confirmed_indices]
    #print confirmed_names
    df_rank_confirmed = pd.DataFrame(confirmed_names)
...
def RandomForest_fs(X_train, y_train):
    ...
    print "the CV score for the best params is", CV_rfc.best_score_
    
    forest = CV_rfc.best_estimator_
    importances = forest.feature_importances_
    #print "the feature importance is", importances
...
def SVC_fs(X_train, y_train):
    estimator= SVC(kernel = "linear", probability= True) #define estimator for feature selection technique
    ...
    print "the number of selected features are", CV_rfc.best_estimator_.n_features_
    ...
    feature_names = X_train.columns.values 
    df_rank = pd.DataFrame({'Rank': CV_rfc.best_estimator_.ranking_, 'Features': feature_names})
    
    #Discerning ranking amongst top features. 
...
def LogisticRegression_Lasso_fs(X_train, y_train):
    ...
    print "the number of features selected is", len(top_indices[1])
