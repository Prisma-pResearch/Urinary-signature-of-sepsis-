...
alias2symbol_transform <- functions(data){
  a2s.obj <- alias2SymbolUsingNCBI(data$Name, 
                                   "./4 Resources/Homo_sapiens.gene_info", 
...
heatmap_sorted <- function(exp_mat, total){
  data_sepsis <- exp_mat[,grepl("P", colnames(exp_mat))]
  data_control <- exp_mat[,grepl("N", colnames(exp_mat))]
  data_sepsis_n <- data_sepsis[, order(colSums(-data_sepsis))]
  data_control_n <- data_control[, order(colSums(-data_control))]
  index_order = c(names(data_control_n), names(data_sepsis_n))
  data_hm = exp_mat[,index_order]
  y <- c(replicate(ncol(data_control), "Vascular"), 
         replicate(ncol(data_sepsis), "Sepsis"))
  ...
  ha = HeatmapAnnotation(data.frame(total$Cell.Specificity), 
                         col = list(total.Cell.Specificity = c("B Cell" = "mediumblue", 
                                                               "Dendritic Cell" = "chartreuse3", 
                                                               "Monocyte" = "slategray3", 
                                                               "Neutrophil" =  "pink2", 
                                                               "NK Cell" = "orange", 
                                                               "T Cell" = "red")),
                         annotation_legend_param = list(total.Cell.Specificity = list(title = "Immune cell type", 
...
aggregate_data <- function(total, exp_mat){
  data <- cbind(total$Cell.Specificity, exp_mat)
  colnames(data)[1] <- "Cell.Specificity"  #Add Cell.specificity to the front of expression values
  aggdata <- aggregate(data, list(total$Cell.Specificity), FUN=mean) 
  aggdata$Cell.Specificity <- aggdata$Group.1
  aggdata$Group.1<-NULL
  rownames(aggdata) <- aggdata$Cell.Specificity
  aggdata$Cell.Specificity <- NULL
  aggdata <- aggdata[,c(colnames(aggdata)[grepl("P", colnames(aggdata))], 
                        colnames(aggdata)[grepl("N", colnames(aggdata))])]  #reordering colnames as "Cell.Specificity" -> "Sepsis" -> "Vascular"
  return(aggdata)
}
...
avg_exp_plt <- function(aggdata){
  ...
  colNew <- c(replicate(table(grepl("P", colnames(aggdata)))["TRUE"], "Sepsis"), 
              replicate(table(grepl("N", colnames(aggdata)))["TRUE"], "Vascular")); 
  ...
  ggplot(data = df, aes(x = variable, y = value, fill = Outcome)) + 
         geom_boxplot(size = 0.75, color = "black", outlier.color = "black", outlier.shape = 16, outlier.size = 1, outlier.stroke = 2) + 
         scale_fill_manual(breaks = df$Outcome, values = unique(as.character(df$color.codes))) + 
         scale_x_discrete(name = "Immune cell type") + 
         scale_y_continuous(name = "Average expression of cell transcript", breaks = seq(4, 7.5, 0.25), limits = c(4, 7.5)) +
         theme_bw() + 
...
dir_reg_plt <- function(total){
  ...
  temp <- data.frame(Cell.Specificity = brplot$Cell.Specificity, 
                     down.regulated = (brplot$Fold.Change<0), 
                     up.regulated = (brplot$Fold.Change>0))
  ...
  ggplot(final_binary, aes(Cell.Specificity)) + 
         ...
         geom_bar(aes(y=up.regulated), stat = "identity", fill = "indianred", width = 0.3) + 
         geom_hline(yintercept = 0, size = 2, color = "grey") +  #+ scale_y_continuous(labels = commapos)
         scale_x_discrete(name = "Immune cell type") + 
         scale_y_continuous(name = "Number of probes in transcript", breaks = seq(-65, 65, 5), limits = c(-65, 65)) +
         theme_bw() + 
         theme(axis.text = element_text(size = 11), axis.title = element_text(size=12, face = "bold")) + 
         theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  dev.off()
