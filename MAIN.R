...
probe_cell_uniquePID <- probe2cell(fc_pval_known_unnest, a2s.obj_clean)
...
exp_matrix_diff <- read.csv("./1 Data/TAC software/Expression matrices/Expression matrix_diffexp_noUTI.csv", header = TRUE, row.names = 1)
fc_pval_diff <- read.csv("./1 Data/TAC software/Fold Change_P Val/Limma_FDR0.01_logFC1.csv", header = TRUE, row.names = 1)
...
write.table(newdata_ipa, "./1 Data/IPA/IPA_TAC_RMA_selectedprobes.txt", quote = FALSE, sep="\t")
